"""scripts/run_cross_sectional_skewness_trial.py -- Phase 4 sq-016.

Runs the CrossSectionalSkewness long-only bottom-quintile
expected-idiosyncratic-skewness strategy on a 10-symbol crypto
universe at 1D (BTC = market factor; 9 alts traded) through
dev_cpcv (run_cpcv_multi), computes DSR + verdict, and appends one
...
from strategies.cross_sectional_skewness import CrossSectionalSkewnessStrategy


STRATEGY_ID = "CrossSectionalSkewness"
VARIATION_ID = "cs-expected-idio-skewness"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-016 Variation #1: long-only bottom-quintile expected "
    "idiosyncratic skewness on a 10-symbol crypto basket (BTC market "
    "factor + 9 alts) at 1D. Daily idiosyncratic returns from "
    "regressing each alt on BTC over a 60-day lookback. Cross-"
    "sectional regression is_t = g0 + g1*is_{t-1} + g2*iv_{t-1} "
    "fitted across alts every 30 days; predicted E[is_{t+1}] = g0 "
    "+ g1*is_t + g2*iv_t ranks the universe ascending. Long the "
    "bottom-2 alts equal-weight (the low-expected-skew long leg), "
    "hold for 30 days. Long-only adaptation of the long-short spec "
    "(Han et al. 2024 -- crypto loser-shorts get punished by "
    "rebound). Sources: Liu/Chen (2024); Tekulova (2022); "
    "Boyer/Mitton/Vorkink (2009); Han/Kang/Ryu (2024)."
)
...
def _build_strategy(symbols: list[str]) -> CrossSectionalSkewnessStrategy:
    """Strategy factory: fresh instance with reset internal state."""
    return CrossSectionalSkewnessStrategy(
        symbols=symbols,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-016 -- CrossSectionalSkewness trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-skewness-dev-headline",
    ...
    # market factor used to extract idiosyncratic residuals; it is
    # NOT a traded asset and so is the natural counterfactual for
    # "did the low-expected-skew alt basket beat passive BTC".
    ...
    notes = (
        "Phase 4 sq-016 variation #1. Long-only bottom-quintile "
        "expected-idiosyncratic-skewness basket on 10-symbol 1D "
        "universe (BTC market factor + 9 alts). Each rebalance bar "
        "(every 30 days) regresses each alt against BTC over the "
        "last 60 days for is_t / iv_t and over the prior 60 days "
        "(offset 30) for is_{t-1} / iv_{t-1}; cross-sectional OLS "
        "predicts E[is_{t+1}] and longs the bottom-2 (lowest "
        "expected skewness) equal-weight. BTC is reference-only -- "
        "always HOLD. Long-only adaptation per Han et al. (2024). "
        "Engine handles 1/N sizing via position_fraction(). "
        ...
        "Sources: Liu/Chen (2024); Tekulova (2022); "
        "Boyer/Mitton/Vorkink (2009); Han/Kang/Ryu (2024)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
