"""scripts/run_daily_cross_sectional_reversal_trial.py -- Phase 4 sq-026.

Runs the DailyCrossSectionalReversal long-only loser-portfolio strategy
on a 9-alt crypto basket at 1D (BTC excluded by design) through
...
from strategies.daily_cross_sectional_reversal import (
    DailyCrossSectionalReversalStrategy,
)


STRATEGY_ID = "DailyCrossSectionalReversal"
...
VARIATION_ID = "cs-daily-reversal"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-026 Variation #1: daily cross-sectional reversal on a "
    "9-alt crypto basket at 1D (BTC excluded as the most-liquid asset "
    "where reversal can flip to momentum). Each bar rank symbols by "
    "prior 1-day return ascending; long the bottom quintile (top_n=2 "
    "of 9, equal weight) -- the 'loser' portfolio. Long-only "
    "adaptation of the long-short specification (engine_multi is "
    "long-only by construction; Han et al. 2024 document loser-shorts "
    "are punished by rebound). Sources: Zaremba (2021); "
    "Caporale/Plastun (2019); Ficura (2023)."
)
...
def _build_strategy(symbols: list[str]) -> DailyCrossSectionalReversalStrategy:
    """Strategy factory: fresh instance with reset internal `_held` set."""
    return DailyCrossSectionalReversalStrategy(
        symbols=symbols,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-026 -- DailyCrossSectionalReversal trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-daily-reversal-dev-headline",
    ...
    # Baseline = ETH/USDT B&H over the same dev window. BTC is
    # intentionally excluded from the traded universe per the
    # hypothesis-of-record (less-liquid focus); ETH is the largest-cap
    # traded alt and the natural counterfactual for "did the loser
    # quintile beat just buying the most-liquid alt that is in the
    # universe".
    ...
    notes = (
        "Phase 4 sq-026 variation #1. Long-only daily cross-sectional "
        "reversal basket on 9-alt 1D universe (BTC excluded by design "
        "per Zaremba 2021 / Ficura 2023 less-liquid focus). Each bar "
        "rank symbols by prior 1-day return ascending; hold bottom-2 "
        "(loser quintile, equal weight). Engine handles 1/N sizing "
        "via position_fraction(). Long-short adaptation reduced to "
        "long leg (engine_multi long-only; Han 2024 documents "
        "loser-short rebound losses). Baseline = ETH/USDT B&H "
        "(largest-cap traded alt; BTC not in universe). "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. "
        "Sources: Zaremba (2021); Caporale/Plastun (2019); "
        "Ficura (2023); Han et al. (2024)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
        "variation_id": VARIATION_ID,
