"""scripts/run_crypto_sector_rotation_trial.py -- Phase 4 sq-029.

Runs the CryptoSectorRotation long-only top-sector strategy on an
11-symbol crypto universe at 1D through dev_cpcv (run_cpcv_multi),
computes DSR + verdict, and appends one trial_type='full_cpcv' row
to backtest/trials.log via the schema-validating writer.  No
holdout access, no commit, no deployment.
...
from strategies.crypto_sector_rotation import CryptoSectorRotationStrategy


STRATEGY_ID = "CryptoSectorRotation"
...
VARIATION_ID = "crypto-sector-rotation-long-only"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-029 Variation #1: long-only sector rotation on an "
    "11-symbol crypto universe at 1D. Each rebalance bar (every 7 "
    "days) computes the equal-weight average prior 30-day return "
    "across three predefined sectors (L1_SmartContract, "
    "Payment_StoreOfValue, DeFi_Oracle), rotates into the single "
    "top-ranked sector, and holds its members equal-weight for the "
    "next 7 days. Long-only; no shorts on losing sectors (Han et al. "
    "2024 -- crypto momentum concentrates in winners). Sources: "
    "Drogen et al. (2023); Han/Kang/Ryu (2024); Grobys (2025)."
)
...
def _build_strategy(symbols: list[str]) -> CryptoSectorRotationStrategy:
    """Strategy factory: fresh instance with reset internal state."""
    return CryptoSectorRotationStrategy(
        symbols=symbols,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-029 -- CryptoSectorRotation trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-sector-rotation-dev-headline",
    ...
    # hypothesis-of-record (Drogen et al. 2023) frames the long-only
    # winner-momentum basket against BTC buy-and-hold; BTC is the
    # explicit counterfactual for crypto cross-sectional studies.
    ...
    notes = (
        "Phase 4 sq-029 variation #1. Long-only sector rotation on "
        "11-symbol 1D universe partitioned into 3 sectors "
        "(L1_SmartContract: ETH/SOL/BNB/AVAX/ADA/DOT; "
        "Payment_StoreOfValue: BTC/XRP/LTC; "
        "DeFi_Oracle: UNI/LINK). Each rebalance bar (every 7 days) "
        "ranks sectors by equal-weight mean prior 30-day return "
        "DESCENDING and rotates into the single top-ranked sector, "
        "holding its members equal-weight for 7 days. Engine handles "
        "sizing via position_fraction = 1/target_sector_size. "
