"""scripts/run_supertrend_phase4a_trial.py — Phase 4.A drop-in batch, Supertrend trial #1.

Runs the daily-resurrection variation through dev_cpcv only, computes the
verdict, and appends one full_cpcv row to backtest/trials.log via the
schema-validating writer.  No commit, no holdout access — that's the
human-only step after review.
"""
...
HYPOTHESIS_TEXT = (
    "Phase 4.A variation #1: daily TF + Barroso-Santa-Clara (2015) "
    "vol-scaling + 6-regime gate to STRONG_BULL/BULL only.  Source: "
    "research/supertrend-literature.md (committed bf4b9ca)."
)
...
PARAMS: dict = {
    ...
    "regime_gate": True,
    "regime_buckets_long": ["STRONG_BULL", "BULL"],
    "vol_scaling": True,
...
def make_strategy() -> SupertrendStrategy:
    return SupertrendStrategy(
        symbol="ETH/USDT",
        timeframe="1d",
        atr_period=10,
        multiplier=2.5,
        daily_resurrection=True,
        target_vol_annual=0.15,
        vol_lookback_days=30,
        vol_factor_cap=1.0,
...
def main() -> int:
    print("=" * 72)
    print("Phase 4.A drop-in batch — Supertrend trial #1 (dev_cpcv only)")
    print("=" * 72)
    ...
    print(f"dev frame: rows={len(dev_df)} range={dev_df.index[0]} → {dev_df.index[-1]}")
    ...
    headline_result = headline_engine.run(
        ...
        period_label="phase4a-supertrend-dev-headline",
    )
    ...
    config = CPCVConfig(n_blocks=10, k_held_out=2, purge_periods=0, embargo_periods=0)
    ...
    # 3. DSR on validation (dev) ─────────────────────────────────────────────
    # NOTE: count_trials_for_dsr returns the count BEFORE this trial is
    # appended; record_trial below will increment it.  The DSR computed
    # here uses the pre-append N (Bailey & López de Prado convention:
    # DSR is per-strategy multiple-testing inflation as of trial draw).
    ...
    # 4. Verdict (dev-side preview using CPCV concat returns) ────────────────
    valid_returns = [r for r in cpcv_result.per_block_returns if r.size > 0]
    concat_returns = np.concatenate(valid_returns) if valid_returns else np.array([])
    ...
    print(f"trade_count_pass        = {verdict.trade_count_pass} (total_trades={verdict.total_trades})")
    ...
    print(f"baseline_sharpe_at_eval = {verdict.baseline_sharpe_at_eval:.4f}")
    ...
    # 5. Append trial row via the writer ─────────────────────────────────────
    event = {
        "strategy_id": STRATEGY_ID,
        ...
        "notes": (
            "Phase 4.A drop-in batch variation #1.  Daily TF "
            "(internal resample of manifest's 1h frame) + Barroso & "
            "Santa-Clara (2015) vol-scaling + 6-regime gate "
            "(STRONG_BULL or BULL).  See research/supertrend-literature.md."
        ),
    }
