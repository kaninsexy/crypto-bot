"""scripts/run_altcoin_season_rotation_trial.py -- AltcoinSeasonRotation trial.

Runs the AltcoinSeasonRotation long-only altcoin-season-vs-BTC switch
strategy on the 11-symbol crypto universe at 1D through dev_cpcv
...
from strategies.altcoin_season_rotation import AltcoinSeasonRotationStrategy


STRATEGY_ID = "AltcoinSeasonRotation"
VARIATION_ID = "altcoin-season-benchmark-rotation"

HYPOTHESIS_TEXT = (
    "Variation #1: long-only altcoin-season vs BTC-benchmark rotation "
    "on an 11-symbol crypto universe at 1D. Each rebalance bar (every "
    "7 days) ranks the 10 altcoins (ETH/SOL/BNB/XRP/ADA/AVAX/DOT/LINK/"
    "LTC/UNI) by prior 30-day return DESCENDING. If the top quintile "
    "(top 2 of 10) mean return exceeds BTC's prior 30-day return, "
    "rotate into the top-quintile alts equal-weight. Otherwise hold "
    "BTC alone. Long-only; no shorts on losing alts (Han et al. 2024 "
    "-- crypto momentum concentrates in winners). Sources: "
    "Drogen/Hoffstein/Otte (2023); Han/Kang/Ryu (2024); Kakushadze/Yu "
    "(2019)."
)
...
def _build_strategy(symbols: list[str]) -> AltcoinSeasonRotationStrategy:
    """Strategy factory: fresh instance with reset internal state."""
    return AltcoinSeasonRotationStrategy(
        symbols=symbols,
        timeframe=PARAMS["timeframe"],
        lookback_period=PARAMS["lookback_period"],
        holding_period=PARAMS["holding_period"],
        top_quintile_pct=PARAMS["top_quintile_pct"],
...
def main() -> int:
    print("=" * 72)
    print(
        "AltcoinSeasonRotation -- variation #1 trial (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="altcoin-season-rotation-dev-headline",
    ...
    # Baseline = BTC/USDT B&H over the same dev window.  Drogen et al.
    # (2023) frame the long-only altcoin-momentum basket against BTC
    # buy-and-hold; BTC is the explicit counterfactual the rotation
    # must beat to justify the alt allocation.
    ...
    notes = (
        "AltcoinSeasonRotation variation #1. Long-only altcoin-season "
        "vs BTC-benchmark rotation on 11-symbol 1D universe (BTC + "
        "10 alts: ETH/SOL/BNB/XRP/ADA/AVAX/DOT/LINK/LTC/UNI). Each "
        "rebalance bar (every 7 days) ranks alts by prior 30-day "
        "return DESCENDING; if the top-quintile (top 2 of 10) mean "
        "return exceeds BTC's prior 30-day return the regime is "
        "ALT_SEASON (long top-quintile equal-weight 1/2), else "
        "BTC_BENCHMARK (long BTC alone at 100%). Engine handles "
        "sizing via position_fraction = 1/target_basket_size. "
        ...
        "Sources: Drogen/Hoffstein/Otte (2023); Han/Kang/Ryu (2024); "
        "Kakushadze/Yu (2019)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
