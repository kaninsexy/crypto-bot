"""scripts/run_cross_sectional_residual_reversal_trial.py -- Phase 4 sq-028.

Runs the CrossSectionalResidualReversal long-only loser-portfolio
strategy on a 9-alt crypto basket at 1D (BTC/USDT included as the
market-factor regressor only -- never traded) through dev_cpcv
...
from strategies.cross_sectional_residual_reversal import (
    CrossSectionalResidualReversalStrategy,
)


STRATEGY_ID = "CrossSectionalResidualReversal"
VARIATION_ID = "cs-residual-reversal-daily"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-028 Variation #1: cross-sectional residual reversal on "
    "a 9-alt crypto basket at 1D, with BTC/USDT as the market-factor "
    "regressor (BTC in symbols but never traded). Each bar fit a "
    "30-day rolling OLS beta of each alt against BTC log-returns; "
    "compute the most-recent 1-day residual = alt_ret - beta * btc_ret; "
    "rank alts ascending; long the bottom-2 (loser quintile of 9 alts, "
    "equal weight). Long-only adaptation of the long-short residual-"
    "reversal specification (engine_multi long-only; Han et al. 2024 "
    "documents loser-shorts are punished by rebound). Sources: "
    "Zaremba (2021); Blitz et al. (2013); Brogaard et al. (2024)."
)
...
def _build_strategy(symbols: list[str]) -> CrossSectionalResidualReversalStrategy:
    """Strategy factory: fresh instance with reset internal `_held` set."""
    return CrossSectionalResidualReversalStrategy(
        symbols=symbols,
        btc_symbol="BTC/USDT",
        timeframe=PARAMS["timeframe"],
        beta_window=PARAMS["beta_window"],
        residual_lookback=PARAMS["residual_lookback"],
        top_n=PARAMS["top_n"],
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-028 -- CrossSectionalResidualReversal trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-residual-reversal-dev-headline",
    ...
    # Baseline = ETH/USDT B&H over the same dev window. BTC is the
    # regressor leg only and is not a traded asset for this strategy;
    # ETH is the largest-cap traded alt and the natural counterfactual
    # for "did the residual-loser quintile beat just buying the
    # most-liquid traded alt".
    baseline_df = data["ETH/USDT"]
    ...
    notes = (
        "Phase 4 sq-028 variation #1. Long-only cross-sectional "
        "residual-reversal basket on 9-alt 1D universe with BTC/USDT "
        "as the market-factor regressor (BTC in symbols but never "
        "traded). Each bar fit a 30-day rolling OLS beta of each alt "
        "vs BTC log-returns; compute most-recent 1-day residual; "
        "rank alts ascending; hold bottom-2 (loser residual quintile, "
        "equal weight). Engine handles 1/N sizing via "
        "position_fraction(). Long-short adaptation reduced to long "
        "leg (engine_multi long-only; Han 2024 documents loser-short "
        "rebound losses). Baseline = ETH/USDT B&H (largest-cap traded "
        "alt; BTC is the regressor leg, not traded). "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. "
        "Sources: Zaremba (2021); Blitz et al. (2013); Brogaard et al. "
        "(2024); Han et al. (2024)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
