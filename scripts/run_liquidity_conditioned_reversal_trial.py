"""scripts/run_liquidity_conditioned_reversal_trial.py -- Phase 4 sq-033.

Runs the LiquidityConditionedReversal long-only loser-portfolio
strategy on the bottom-quintile-by-dollar-volume subset of an
...
from strategies.liquidity_conditioned_reversal import (
    LiquidityConditionedReversalStrategy,
)


STRATEGY_ID = "LiquidityConditionedReversal"
...
VARIATION_ID = "liquidity-conditioned-reversal-daily"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-033 Variation #1: cross-sectional short-term reversal "
    "applied only to the bottom-quintile-by-dollar-volume (illiquid) "
    "subset of an 11-symbol crypto basket at 1D. Each bar rank symbols "
    "by 30-day mean dollar volume ascending; take bottom-2 (most "
    "illiquid). Within that subset rank by 1-day prior return "
    "ascending; long bottom-1 (the biggest loser). Long-only; daily "
    "rebalance. Sources: Ficura/Colak (2023); Zaremba (2021); "
    "Wen/Bouri/Xu/Zhao (2022)."
)

PARAMS: dict = {
    "liquidity_lookback": 30,
    "liquidity_bottom_n": 2,
    "reversal_lookback": 1,
    "top_n": 1,
...
def _build_strategy(symbols: list[str]) -> LiquidityConditionedReversalStrategy:
    """Strategy factory: fresh instance with reset internal `_held` set."""
    return LiquidityConditionedReversalStrategy(
        symbols=symbols,
        timeframe=PARAMS["timeframe"],
        liquidity_lookback=PARAMS["liquidity_lookback"],
        liquidity_bottom_n=PARAMS["liquidity_bottom_n"],
        reversal_lookback=PARAMS["reversal_lookback"],
        top_n=PARAMS["top_n"],
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-033 -- LiquidityConditionedReversal trial #1 "
        "(dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-liquidity-conditioned-reversal-dev-headline",
    ...
    # hypothesis claims the long-only liquidity-conditioned loser
    # basket outperforms BTC buy-and-hold; BTC is the explicit
    # counterfactual the cross-sectional reversal literature uses.
    ...
    notes = (
        "Phase 4 sq-033 variation #1. Long-only liquidity-conditioned "
        "cross-sectional reversal on 11-symbol 1D universe. Each bar "
        "rank by 30-day mean dollar volume ascending; take bottom-2 "
        "(illiquid quintile). Within that subset rank by 1-day prior "
        "return ascending; hold bottom-1 (single biggest loser). "
        ...
        "Sources: Ficura/Colak (2023); Zaremba (2021); "
        "Wen/Bouri/Xu/Zhao (2022)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
