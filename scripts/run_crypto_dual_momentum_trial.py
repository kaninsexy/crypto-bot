"""scripts/run_crypto_dual_momentum_trial.py -- Phase 4 sq-032.

Runs the CryptoDualMomentum (TSMOM filter + cross-sectional rotation)
...
from strategies.crypto_dual_momentum import CryptoDualMomentumStrategy


STRATEGY_ID = "CryptoDualMomentum"
VARIATION_ID = "tsmom-filter-cs-rotation"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-032 Variation #1: dual-momentum on a 10-symbol "
    "crypto basket at 1D. TSMOM market filter (BTC/USDT close > "
    "200-day SMA) gates entries; when ON, rank symbols by prior "
    "180-day return descending and long the top quintile (top_n=2 "
    "of 10, equal weight) for a 30-day holding window. When OFF, "
    "liquidate held and stay flat. Long-only -- no shorts on "
    "losers (Han et al. 2024). Sources: Han/Kang/Ryu (2024); "
    "Borgards (2021); Huang/Sangiorgi/Urquhart (2024)."
)
...
def _build_strategy(symbols: list[str]) -> CryptoDualMomentumStrategy:
    """Strategy factory: fresh instance with reset internal state."""
    return CryptoDualMomentumStrategy(
        symbols=symbols,
        timeframe=PARAMS["timeframe"],
        market_filter_symbol=PARAMS["market_filter_symbol"],
        tsmom_lookback=PARAMS["tsmom_lookback"],
        cs_lookback=PARAMS["cs_lookback"],
        top_n=PARAMS["top_n"],
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-032 -- CryptoDualMomentum trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-crypto-dual-momentum-dev-headline",
    ...
    try:
        ...
        cpcv_error_event = {
            ...
            "notes": (
                ...
                "rotation events across blocks under the TSMOM filter."
            ),
        }
    ...
    # explicit absolute counterfactual for "did the dual-momentum
    # rotation beat just holding the market index?" -- consistent
    # with Han et al. (2024) framing TSMOM strategies against BTC.
    ...
    notes = (
        "Phase 4 sq-032 variation #1. Dual-momentum on 10-symbol 1D "
        "crypto basket. TSMOM filter (BTC/USDT close vs 200-day SMA) "
        "gates entries: filter ON -> CS rotation into top-2 winners "
        "ranked by 180-day return; filter OFF -> exit-to-flat and "
        "no entries. 30-day holding period (monthly rebalance). "
        "Equal weight 1/top_n via position_fraction(). Long-only "
        "(Han et al. 2024 -- losers rebound). Baseline = BTC/USDT "
        "B&H. "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. "
        "Sources: Han/Kang/Ryu (2024); Borgards (2021); "
        "Huang/Sangiorgi/Urquhart (2024)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
        "variation_id": VARIATION_ID,
