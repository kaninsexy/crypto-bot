"""scripts/run_trendfollowing_multi_phase4a_trial.py — Phase 4.A drop-in batch,
TrendFollowing_multi trial #1 (variation phase4a-hop-daily-multi-v1).

Runs the daily TSMOM 11-instrument basket through the dev_cpcv harness
(run_cpcv_multi), computes the DSR + verdict, and appends one
...
Structure mirrors run_supertrend_phase4a_trial.py with multi-asset
adaptations (Option-C dispatch — load_dev returns a concatenated
DataFrame with a 'symbol' column; this script pivots into the per-
symbol dict that run_cpcv_multi consumes).
"""
...
HYPOTHESIS_TEXT = (
    "Phase 4.A variation #1: daily TSMOM on 11-instrument crypto basket. "
    "Signal = sign of trailing 126-day return per instrument "
    "(Hurst/Ooi/Pedersen 2017 6-month formation). "
    "Vol-targeting per instrument: (target_vol_annual / realized_vol_126d) "
    "x (1/N_active) x portfolio_capital (Barroso & Santa-Clara 2015). "
    "Rebalance daily. Long-only spot. Basket read from manifest at runtime. "
    "Source: research/trendfollowing-literature.md (committed d3265c2)."
)

# PARAMS dict: keys mirror TrendFollowingMultiStrategy.__init__ keyword
# arguments exactly (timeframe, lookback_days, target_vol_annual).  The
# `symbols` constructor argument is intentionally absent — symbols come
# from the manifest at runtime, never hardcoded in the params.
PARAMS: dict = {
    "timeframe": "1d",
    "lookback_days": 126,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4.A drop-in batch — TrendFollowing_multi trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_strategy = TrendFollowingMultiStrategy(
        symbols=symbols,
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4a-tf-multi-dev-headline",
    ...
    cpcv_strategy = TrendFollowingMultiStrategy(
        symbols=symbols,
        timeframe=PARAMS["timeframe"],
        lookback_days=PARAMS["lookback_days"],
        target_vol_annual=PARAMS["target_vol_annual"],
    ...
    # First full_cpcv for TrendFollowing_multi → returns 0 (smoke
    # excluded).  dsr_from_cpcv_result calls that helper internally and
    # the DSRError raised at n_trials < 1 short-circuits.  Mirror the
    # phase_4b_full_cpcv_v1.py pattern and monkeypatch the lookup so
    ...
    # Baseline = BTC/USDT B&H over the same dev window.  Largest-cap
    # instrument in the basket; comparable for "did the basket beat
    # buy-and-hold the most-relevant single asset".
    ...
    # ── 6. Append trial row ─────────────────────────────────────────────────
    notes = (
        "Phase 4.A drop-in batch variation #1. Daily TSMOM on 11-symbol "
        "OKX spot basket. 126-day formation (Hurst/Ooi/Pedersen 2017). "
        "Barroso & Santa-Clara (2015) vol-scaling per instrument. "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. Basket read from manifest at runtime. "
        "MATIC/USDT excluded: renamed to POL Sept 2024, insufficient "
        "history. Baseline: BTC/USDT B&H. "
        "See research/trendfollowing-literature.md."
    )
    event = {
        "strategy_id": STRATEGY_ID,
