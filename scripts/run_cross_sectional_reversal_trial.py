"""scripts/run_cross_sectional_reversal_trial.py -- Phase 4 sq-013.

Runs the CrossSectionalReversal long-only loser-portfolio strategy on
...
STRATEGY_ID = "CrossSectionalReversal"
...
VARIATION_ID = "cs-short-term-reversal-loser-portfolio"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-013 Variation #1: cross-sectional short-term reversal "
    "on a 10-symbol crypto basket at 1D. Each bar rank symbols by "
    "prior 1-day return ascending; long the bottom quintile (top_n=2 "
    "of 10, equal weight) -- the 'loser' portfolio. Long-only; "
    "rebalance every bar. Sources: Zaremba (2021); Nakagawa/Sakemoto "
    "(2024); Han/Kang/Ryu (2023)."
)
...
def _build_strategy(symbols: list[str]) -> CrossSectionalReversalStrategy:
    """Strategy factory: fresh instance with reset internal `_held` set."""
    return CrossSectionalReversalStrategy(
        symbols=symbols,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-013 -- CrossSectionalReversal trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-reversal-dev-headline",
    ...
    # hypothesis-of-record claims the long-only loser-quintile basket
    ...
    notes = (
        "Phase 4 sq-013 variation #1. Long-only cross-sectional "
        "short-term reversal basket on 10-symbol 1D universe. Each "
        "bar rank symbols by prior 1-day return ascending; hold "
        "bottom-2 (loser quintile, equal weight). Engine handles 1/N "
        "sizing via position_fraction(). Baseline = BTC/USDT B&H. "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. "
        "Sources: Zaremba (2021); Nakagawa/Sakemoto (2024); "
        "Han/Kang/Ryu (2023)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
