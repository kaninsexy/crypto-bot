"""scripts/run_meanreversion_btc_residual_phase4a_trial.py — Phase 4.A
MeanReversion BTC-Residual trial #1 (variation
phase4a-btc-residual-mr-v1).

Runs the BTC-neutral residual mean-reversion strategy on a 5-alt
basket through the dev_cpcv harness (run_cpcv_multi), computes DSR +
...
Structure mirrors run_trendfollowing_multi_phase4a_trial.py — the
authoritative multi-symbol Phase 4.A template.
"""
...
from strategies.mean_reversion_btc_residual import (
    MeanReversionBTCResidualStrategy,
)
...
HYPOTHESIS_TEXT = (
    "Phase 4.A variation #1: BTC-neutral residual mean-reversion on "
    "5-alt basket (ETH/SOL/BNB/XRP/ADA) at 4H. Rolling 60-bar OLS "
    "beta, 30-bar cumulative residual z-score, entry z<-1.5, exit z=0 "
    "or -8% stop or 30-bar hold cap, max 2 concurrent positions. "
    "Source: Fil & Kristoufek (2020) IEEE Access."
)

# PARAMS dict: keys mirror MeanReversionBTCResidualStrategy.__init__
# keyword arguments exactly. The `symbols` and `btc_symbol`
# constructor arguments are intentionally absent — symbols come from
# the manifest at runtime; btc_symbol is structural and never tuned.
PARAMS: dict = {
    "beta_window": 60,
    "zscore_window": 30,
...
def _build_strategy(symbols: list[str]) -> MeanReversionBTCResidualStrategy:
    return MeanReversionBTCResidualStrategy(
        symbols=symbols,
        btc_symbol="BTC/USDT",
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4.A — MeanReversion_BTC_Residual trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4a-mr-btc-resid-dev-headline",
    ...
    # First full_cpcv for MeanReversion_BTC_Residual → returns 0
    # (smoke excluded). dsr_from_cpcv_result calls that helper
    # internally and the DSRError raised at n_trials < 1 short-
    # circuits.  Mirror the TrendFollowing-multi pattern and
    # monkeypatch the lookup so the deflation runs against
    # n_trials=1.
    ...
    # hedge leg being regressed out; it is NOT a traded asset and
    # therefore not the right counterfactual.  ETH is the largest-cap
    # traded alt and the natural counterfactual for "did residual MR
    # beat just buying the most-liquid alt".
    ...
    # ── 6. Append trial row ─────────────────────────────────────────────────
    notes = (
        "Phase 4.A variation #1. BTC-neutral residual mean-reversion "
        "on 5-alt basket (ETH/SOL/BNB/XRP/ADA) at 4H. 60-bar OLS beta "
        "vs BTC, 30-bar cumulative residual z-score (cross-sectional), "
        "entry z<-1.5, exit z=0 or -8% stop or 30-bar hold cap, max 2 "
        "concurrent positions. BTC is the reference leg only — never "
        "traded; ETH/USDT is the verdict baseline. "
