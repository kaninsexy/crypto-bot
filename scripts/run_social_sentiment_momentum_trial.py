...
Runs the SocialSentimentMomentum strategy through the dev_cpcv harness
with LunarCrush Galaxy Score injected as a `sentiment` column on each
per-symbol OHLCV frame.
...
Exit codes:
  ...
  2 — sentiment data cache absent (actionable: run the fetcher)
...
HYPOTHESIS_TEXT = (
    "sq-002 variation #1: aggregated social sentiment momentum on a "
    "crypto basket. LunarCrush Galaxy Score (0-100) injected as a "
    "`sentiment` column alongside OHLCV; long when rolling 7-bar "
    "sentiment mean > 60, flat when < 40. Long-only basket, single "
    "concurrent position per symbol. "
    "Sources: Ortu et al. (2022) Expert Systems 198; Zhang & Zhang "
    "(2022) Research in International Business and Finance 61."
)

# PARAMS dict: keys mirror SocialSentimentMomentumStrategy.__init__
# keyword arguments. `symbols` and `notional_capital` come from the
# manifest entry / engine defaults at runtime.
PARAMS: dict = {
    "sentiment_window": 7,
    "long_threshold": 60.0,
    "flat_threshold": 40.0,
...
LUNARCRUSH_DIR = ROOT / "data" / "lunarcrush"
SENTIMENT_PARQUET_SUFFIX = "_galaxy.parquet"
SENTIMENT_RAW_COLUMN = "galaxy_score"
SENTIMENT_INJECTED_COLUMN = "sentiment"


def _build_strategy(symbols: list[str]) -> SocialSentimentMomentumStrategy:
...
def _load_sentiment_for(symbol: str) -> pd.Series | None:
    """Read the per-symbol LunarCrush parquet and return a tz-aware
    UTC-indexed Series of the sentiment value (renamed from
    SENTIMENT_RAW_COLUMN to SENTIMENT_INJECTED_COLUMN). Returns None
    when the file is absent or unreadable."""
    fname = symbol.replace("/", "-") + SENTIMENT_PARQUET_SUFFIX
    fpath = LUNARCRUSH_DIR / fname
    if not fpath.exists():
        return None
    try:
        ...
        print(
            f"[sentiment] WARNING: failed to read {fpath}: {exc}",
            file=sys.stderr,
        )
        return None
    if SENTIMENT_RAW_COLUMN not in df.columns:
        print(
            f"[sentiment] WARNING: {fpath} missing column "
            f"'{SENTIMENT_RAW_COLUMN}'",
            file=sys.stderr,
        )
        return None
    if "timestamp" in df.columns:
        df = df.set_index(pd.to_datetime(df["timestamp"], utc=True))
    series = df[SENTIMENT_RAW_COLUMN].astype(float).sort_index()
    series.name = SENTIMENT_INJECTED_COLUMN
    return series


def _check_sentiment_cache(symbols: list[str]) -> dict[str, pd.Series]:
    """Probe LUNARCRUSH_DIR for per-symbol sentiment caches.
    ...
    for sym in symbols:
        s = _load_sentiment_for(sym)
        if s is None:
            print(
                f"[sentiment] no cache for {sym} — skipping",
                file=sys.stderr,
            )
            continue
        out[sym] = s
...
def main() -> int:
    ...
    if len(active_symbols) < len(symbols):
        skipped = [s for s in symbols if s not in sentiment_by_sym]
        print(
            f"[sentiment] basket reduced from {len(symbols)} to "
    ...
    for sym in active_symbols:
        ...
        # Inject sentiment column via reindex+ffill on the OHLCV index.
        sent = sentiment_by_sym[sym].reindex(df.index, method="ffill")
        df = df.copy()
        df[SENTIMENT_INJECTED_COLUMN] = sent.astype(float)
        data[sym] = df
    ...
    if missing:
        ...
        return 1
    print("\nper-symbol dev coverage (with sentiment injected):")
    for sym, df in data.items():
        nan_count = int(df[SENTIMENT_INJECTED_COLUMN].isna().sum())
        print(
            f"  {sym:<12} rows={len(df):>5} "
            f"sentiment_nan={nan_count} "
    ...
    config = CPCVConfig(
        ...
        strategy_warmup_candles=int(
            entry.get("strategy_warmup_candles", PARAMS["sentiment_window"])
        ),
    ...
    # Baseline = BTC/USDT B&H. The sentiment basket is opportunistic;
    # BTC is the natural counterfactual for "did the sentiment overlay
    # add value over passive BTC exposure".
    baseline_df = data.get("BTC/USDT")
    if baseline_df is None:
        # Fall back to the first available symbol if BTC isn't in the
        # active basket (unusual; logged for transparency).
    ...
    # ── 6. Append trial row ─────────────────────────────────────────────────
    notes = (
        "sq-002 variation #1. Social sentiment momentum (LunarCrush "
        "Galaxy Score) on a crypto basket. Long when rolling 7-bar "
        "sentiment mean > 60, flat when < 40. Long-only basket, "
        "single concurrent position per symbol. Baseline: BTC/USDT B&H. "
        f"Warmup-aware CPCV: strategy_warmup_candles="
        f"{config.strategy_warmup_candles}, effective_n_blocks="
        f"{effective_n_blocks}. "
        "Sources: Ortu et al. (2022); Zhang & Zhang (2022)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
