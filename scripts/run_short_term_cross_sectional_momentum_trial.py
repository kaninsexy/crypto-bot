"""scripts/run_short_term_cross_sectional_momentum_trial.py -- Phase 4 sq-022.

Runs the ShortTermCrossSectionalMomentum long-only winner-portfolio
...
from strategies.short_term_cross_sectional_momentum import (
    ShortTermCrossSectionalMomentumStrategy,
)


STRATEGY_ID = "ShortTermCrossSectionalMomentum"
VARIATION_ID = "cs-short-term-mom-7d"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-024 Variation #1: short-term cross-sectional momentum "
    "on a 10-symbol crypto basket at 1D. On each rebalance bar, rank "
    "symbols by prior 7-day return descending; long the top quintile "
    "(top_n=2 of 10, equal weight) -- the 'winner' portfolio -- and "
    "hold for 7 days before the next rebalance. Long-only; no shorts "
    "on losers. Sources: Drogen et al. (2023); Zaremba et al. (2021); "
...
def _build_strategy(symbols: list[str]) -> ShortTermCrossSectionalMomentumStrategy:
    """Strategy factory: fresh instance with reset internal state."""
    return ShortTermCrossSectionalMomentumStrategy(
        symbols=symbols,
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-024 -- ShortTermCrossSectionalMomentum trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-short-term-mom-dev-headline",
    ...
    # hypothesis-of-record (Drogen et al. 2023; Tzouvanas et al. 2020)
    # frames the long-only winner-quintile basket against BTC
    # buy-and-hold; BTC is the explicit counterfactual.
    ...
    notes = (
        "Phase 4 sq-024 variation #1. Long-only short-term cross-"
        "sectional momentum on 10-symbol 1D universe. Each rebalance "
        "bar (every 7 days) ranks symbols by prior 7-day return "
        "descending and holds top-2 (winner quintile, equal weight) "
        "for the next 7 days. Engine handles 1/N sizing via "
        ...
        "Sources: Drogen et al. (2023); Zaremba et al. (2021); "
        "Tzouvanas et al. (2020)."
    )
    event = {
        "strategy_id": STRATEGY_ID,
