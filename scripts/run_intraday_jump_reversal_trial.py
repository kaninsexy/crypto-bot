"""scripts/run_intraday_jump_reversal_trial.py -- IntradayJumpReversal full_cpcv trial.

Runs the IntradayJumpReversal z-score fade variation through
dev_cpcv only on BTC/USDT 1H, computes the verdict, and appends
one trial_type='full_cpcv' row to backtest/trials.log via the
schema-validating writer.  No holdout access -- that's the
...
from strategies.intraday_jump_reversal import (
    IntradayJumpReversalStrategy,
)


STRATEGY_ID = "IntradayJumpReversal"
...
VARIATION_ID = "intraday-jump-reversal-zscore"

HYPOTHESIS_TEXT = (
    "IntradayJumpReversal: fade statistically significant negative "
    "1H jumps on BTC/USDT (z = r_t / rolling_std_24h <= -3.0) and "
    "exit on opposite-direction jump or 24-bar time cap.  Long-only. "
    "Sources: Wen et al. (2022) NAJEF (intraday reversal predictors "
    "produce higher economic value than B&H), Zaremba et al. (2021) "
    "IRFA (low last-day-return cryptos outperform high last-day-"
    "return cryptos; reversal effect strongest in illiquid coins), "
    "Alexzap (2026) Medium/DEV (BTC-USD 2022-2024 z-score JMR "
    "backtest produced positive Sharpe)."
)
...
def make_strategy() -> IntradayJumpReversalStrategy:
    """Fresh instance per call so CPCV block boundaries reset
    `_position_open` to False and `_bars_held` to 0 (CPCV
    correctness gate)."""
    return IntradayJumpReversalStrategy(
        symbol="BTC/USDT",
        timeframe="1h",
...
def _record_cpcv_error_retire(
    ...
    notes = (
        f"IntradayJumpReversal CPCVError: {msg}.  Retire row written "
        "via sentinel handler per .claude/rules/backtest.md."
    )
    event = {
...
def main() -> int:
    print("=" * 72)
    print(
        "IntradayJumpReversal -- intraday-jump-reversal-zscore trial (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = headline_engine.run(
        ...
        period_label="intraday-jump-reversal-dev-headline",
    )
    ...
    # 5. Append trial row.
    notes = (
        "IntradayJumpReversal first full_cpcv. Single-pair BTC/USDT "
        "1H, long-only fade of statistically significant negative 1H "
        "jumps (z = r_t / rolling_std_24h <= -3.0).  Exit on opposite-"
        "direction jump (z >= +3.0) or 24-bar time cap.  Long-only "
        "(_position_open guard: SELL only fires when a prior BUY "
        "flipped state to True). "
        "See research/intraday-jump-reversal-literature.md."
    )
    event = {
        "strategy_id": STRATEGY_ID,
        "variation_id": VARIATION_ID,
    ...
    _trials.record_trial(
        ...
        per_bar_benchmark=(
            primary_baseline_df["close"].pct_change().dropna().values.astype(float)
        ),
    )
    print(
