"""scripts/run_cross_sectional_momentum_trial.py -- Phase 4 sq-020.

Runs the CrossSectionalMomentum long-only winner-portfolio strategy
on a 10-symbol crypto universe at 1D through dev_cpcv
...
STRATEGY_ID = "CrossSectionalMomentum"
...
VARIATION_ID = "cs-momentum-long-winners"

HYPOTHESIS_TEXT = (
    "Phase 4 sq-020 Variation #1: cross-sectional momentum on a "
    "10-symbol crypto basket at 1D. On each rebalance bar, rank "
    "symbols by prior 30-day return descending; long the top "
    "quintile (top_n=2 of 10, equal weight) -- the 'winner' "
    "portfolio -- and hold for 7 days before the next rebalance. "
    "Long-only; no shorts on losers (Han et al. 2024). Sources: "
    "Drogen et al. (2023); Han/Kang/Ryu (2024); Borgards (2021)."
)
...
def _build_strategy(symbols: list[str]) -> CrossSectionalMomentumStrategy:
...
def main() -> int:
    print("=" * 72)
    print(
        "Phase 4 sq-020 -- CrossSectionalMomentum trial #1 (dev_cpcv only)"
    )
    print("=" * 72)
    ...
    headline_result = run_engine_multi(
        ...
        period_label="phase4-cs-momentum-dev-headline",
    ...
    # winner-quintile basket against BTC buy-and-hold; BTC is the
    ...
    notes = (
        "Phase 4 sq-020 variation #1. Long-only cross-sectional "
        "momentum basket on 10-symbol 1D universe. Each rebalance "
        "bar (every 7 days) ranks symbols by prior 30-day return "
        "descending and holds top-2 (winner quintile, equal "
        "weight) for the next 7 days. Engine handles 1/N sizing "
